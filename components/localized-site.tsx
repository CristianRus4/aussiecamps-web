...
export function LocalizedHome({ locale }: { locale: LocaleCode }) {
  ...
  return <>
    ...
    <main>
      <section className="hero"><div className="hero-glow"/><div className="hero-grid"><div className="hero-copy"><span className="kicker"><Caravan size={14}/> {ui.heroKicker}</span><h1>{ui.heroTitle}</h1><p>{ui.heroText}</p><div className="hero-actions"><LocalizedDownloadCard locale={locale}/><Button asChild variant="outline" size="lg" className="rounded-[14px]"><Link href={`${prefix(locale)}#features`}>{ui.seeHow}<ArrowRight/></Link></Button></div><div className="hero-proof"><span><strong>{ui.statPlaces}</strong><small>{ui.statPlacesLabel}</small></span><span><strong>{ui.statOffline}</strong><small>{ui.statOfflineLabel}</small></span><span><strong>{ui.statStays}</strong><small>{ui.statStaysLabel}</small></span></div></div><div className="hero-visual"><ScreenshotImage className="hero-image-slot" src="/images/aussie-hero.webp" alt={ui.altHero} priority sizes="(max-width: 700px) 215px, 320px"/></div></div></section>
      ...
      <section className="download-section"><ScreenshotImage className="download-image" src="/images/aussie-download.webp" alt={ui.altExploreScreen} sizes="(max-width: 700px) 210px, 300px"/><div className="download-copy"><p className="eyebrow">{ui.downloadEyebrow}</p><h2>{ui.downloadTitle}</h2><p>{ui.downloadText}</p><LocalizedDownloadCard locale={locale}/></div></section>
    </main>
