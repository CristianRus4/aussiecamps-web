...
import { ArrowLeft, ArrowRight, ArrowRightLeft, ArrowUpRight, Binoculars, CalendarDays, Calculator, Caravan, Check, CircleDollarSign, Clock, CloudSun, Droplets, Fuel, Heart, Layers, ListChecks, Map, MapPin, MapPinned, Menu, Navigation, Route, Search, ShieldCheck, SlidersHorizontal, TentTree, Trees, Wallet } from "lucide-react";
import { FaApple } from "react-icons/fa6";
import { BrandMark } from "./brand-mark";
import { Button } from "./ui/button";
import { APP_STORE_URL, SUPPORT_EMAIL, type Article } from "@/lib/site";
import { currencyRates, displayCurrencies, exchangeRateDate, formatAudRange } from "@/lib/currency";
...
export function LocalizedHome({ locale }: { locale: LocaleCode }) {
  ...
  const reviews = [["Amélie D.",ui.review1Country,ui.review1Title,ui.review1Quote],["Michael K.",ui.review2Country,ui.review2Title,ui.review2Quote],["Priya S.",ui.review3Country,ui.review3Title,ui.review3Quote],["Lukas H.",ui.review4Country,ui.review4Title,ui.review4Quote],["Oliver P.",ui.review5Country,ui.review5Title,ui.review5Quote]];
  ...
  return <>
    ...
    <main>
      <section className="hero"><div className="hero-glow"/><div className="hero-grid"><div className="hero-copy"><span className="kicker"><Caravan size={14}/> {ui.heroKicker}</span><h1>{ui.heroTitle}</h1><p>{ui.heroText}</p><div className="hero-actions"><div className="download-card"><div className="qr-image" style={{backgroundImage:"url(/images/aussie-qr.webp)"}}/><StoreLink locale={locale}/></div><Button asChild variant="outline" size="lg" className="w-[170px] rounded-[14px]"><Link href={`${prefix(locale)}#features`}>{ui.seeHow}<ArrowRight/></Link></Button></div><div className="hero-proof"><span><strong>{ui.statPlaces}</strong><small>{ui.statPlacesLabel}</small></span><span><strong>{ui.statOffline}</strong><small>{ui.statOfflineLabel}</small></span><span><strong>{ui.statStays}</strong><small>{ui.statStaysLabel}</small></span></div></div><div className="hero-visual"><div className="image-slot hero-image-slot" style={{backgroundImage:"url(/images/aussie-hero.webp)"}}/></div></div></section>

      <section className="trust-strip"><p>{ui.trustLine}</p><div>{[TentTree,Caravan,Trees,Navigation,Map,CloudSun].map((Icon,i)=><span key={i}><Icon/>{[ui.trustCampsites,ui.trustCaravanParks,ui.trustNationalParks,ui.trustRestAreas,ui.trustUsefulStops,ui.trustExperiences][i]}</span>)}</div></section>
      ...
      <section className="feature-stage" id="features"><div className="feature-intro"><p className="eyebrow">{ui.featuresEyebrow}</p><h2>{ui.featuresTitle}</h2><p>{ui.featuresText}</p></div><div className="feature-showcase"><div className="feature-copy-large"><div className="icon-tile"><MapPinned/></div><p className="eyebrow">{ui.featureMainEyebrow}</p><h3>{ui.featureMainTitle}</h3><p>{ui.featureMainText}</p><ul><li><Check/>{ui.featureList1}</li><li><Check/>{ui.featureList2}</li><li><Check/>{ui.featureList3}</li></ul></div><div className="product-images"><div className="image-slot product-shot product-shot-one" style={{backgroundImage:"url(/images/aussie-feature-1.webp)"}}/><div className="image-slot product-shot product-shot-two" style={{backgroundImage:"url(/images/aussie-feature-2.webp)"}}/></div></div><div className="feature-grid">{features.map(([Icon,title,text])=><article key={title}><Icon/><h3>{title}</h3><p>{text}</p></article>)}</div></section>

      <section className="section itinerary-section"><div className="itinerary-image image-slot" style={{backgroundImage:"url(/images/aussie-feature-3.webp)"}}/><div className="itinerary-copy"><p className="eyebrow">{ui.plannerEyebrow}</p><h2>{ui.plannerTitle}</h2><p>{ui.plannerText}</p><div className="metric-grid"><div><Route/><strong>{ui.plannerMetric1Title}</strong><small>{ui.plannerMetric1Text}</small></div><div><CalendarDays/><strong>{ui.plannerMetric2Title}</strong><small>{ui.plannerMetric2Text}</small></div><div><ListChecks/><strong>{ui.plannerMetric3Title}</strong><small>{ui.plannerMetric3Text}</small></div><div><CloudSun/><strong>{ui.plannerMetric4Title}</strong><small>{ui.plannerMetric4Text}</small></div></div></div></section>

      {articleList.length > 0 && <section className="section guide-preview"><div className="guide-preview-head"><div><p className="eyebrow">{ui.routesEyebrow}</p><h2>{ui.routesTitle}</h2></div><div><p>{ui.routesText}</p><Link href={prefix(locale,"/guides")} className="text-link">{ui.exploreRoutes}<ArrowRight/></Link></div></div><div className="article-grid guide-preview-grid">{articleList.slice(0,5).map((article,index)=><LocalizedCard article={article} locale={locale} priority={index===0} key={article.slug}/>)}</div></section>}
      ...
      <section className="download-section"><div className="download-image image-slot" style={{backgroundImage:"url(/images/aussie-download.webp)"}}/><div className="download-copy"><p className="eyebrow">{ui.downloadEyebrow}</p><h2>{ui.downloadTitle}</h2><p>{ui.downloadText}</p><StoreLink locale={locale}/></div></section>
    </main>
...
export function LocalizedGuides({ locale }: { locale: LocaleCode }) {
  ...
  return <><LocalizedHeader locale={locale}/><main><section className="page-hero guides-hero"><p className="eyebrow">{ui.guidesEyebrow}</p><h1>{ui.guidesTitle}</h1><p>{list.length ? `${list.length} ${ui.guidesIntro}` : ui.guidesEmptyText}</p></section><section className="content-shell">{list.length===0 && <section className="category-section"><div className="category-heading"><h2>{ui.guidesEmptyTitle}</h2></div><p><Link href="/guides" className="text-link">{ui.guidesEmptyLink} <ArrowRight/></Link></p></section>}{localizedCategories(locale).map(({source,label})=>{const group=list.filter((article)=>article.category===source);if(!group.length)return null;return <section className="category-section" key={source}><div className="category-heading"><h2>{label}</h2><span>{group.length} {group.length===1?ui.guideSingular:ui.guidePlural}</span></div><div className="article-grid">{group.map((article)=><LocalizedCard article={article} locale={locale} key={article.slug}/>)}</div></section>;})}</section></main><LocalizedFooter locale={locale}/></>;
}
...
const rates=${JSON.stringify(currencyRates)};
...
const aud=(value)=>'A$'+value.toFixed(2);
...
const a=adults*nights*perPerson,b=nights*perSite;root.querySelector('[data-camp-output]').textContent=aud(a)+' vs '+aud(b)+' · '+(a===b?${JSON.stringify(ui.theSame)}:(a<b?${JSON.stringify(ui.perAdultCheaper)}:${JSON.stringify(ui.perSiteCheaper)}))});
})();`;
